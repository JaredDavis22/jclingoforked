...
public class ShowTerm extends Ast {
    ...
    private static Pointer create(Location location, Ast term, AstSequence body) {
        PointerByReference pointerByReference = new PointerByReference();
        Clingo.check(Clingo.INSTANCE.clingo_ast_build(AstType.SHOW_TERM.getValue(), pointerByReference, location, term.getPointer(), body.getPointer(), new NativeSize(body.size())));
