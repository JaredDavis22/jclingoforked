...
public class TheoryTermDefinition extends Ast {
    ...
    public AstSequence getOperators() {
        return new AstSequence(this, AstAttribute.OPERATORS);
    }
    ...
    public void setOperators(AstSequence operators) {
        new AstSequence(this, AstAttribute.OPERATORS).set(operators);
    }

    private static Pointer create(Location location, String name, AstSequence operators) {
        PointerByReference pointerByReference = new PointerByReference();
        Clingo.check(Clingo.INSTANCE.clingo_ast_build(AstType.THEORY_TERM_DEFINITION.getValue(), pointerByReference, location, name, operators.getPointer(), new NativeSize(operators.size())));
